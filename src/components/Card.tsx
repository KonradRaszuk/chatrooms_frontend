...
export default function Card({ title, onJoin, id }: RoomCardProps) {
  ...
  return (
    <div className="room_card">
      ...
      <div
        className={`room_card_manager_wrapper ${
          showManager ? "room_card_manager_wrapper_transition" : ""
        }`}
      >
        <button onClick={handleDelete} className="room_card_manager_btn">
          DELETE
        </button>
        <button onClick={handleEdit} className="room_card_manager_btn">
          EDIT
        </button>
      </div>
    </div>
